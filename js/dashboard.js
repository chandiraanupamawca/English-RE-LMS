...
function searches() {
	...
	for (i = 0; i < nodes.length; i++) {
		if (nodes[i].innerHTML.toLowerCase().includes(filter)) {
			if (document.getElementsByClassName("alltitle")[i].innerText != "Loading Class Name") {
	...
	function v2(d) {
		...
		function v3(dt) {
			...
			for (let i = 0; i < Object.keys(dt).length; i++) {
				...
				document.getElementsByClassName("allcid")[k].innerText = "Class ID: " + cos["section"] + ("000" + Object.keys(dt)[i]).slice(-4)
				document.getElementsByClassName("allbtn")[k].id = Object.keys(dt)[i] + "||" + cos["pn"]
				document.getElementsByClassName("allyr")[k].innerText = cos["year"] + " " + cos["section"]
				...
				if ((ztc).includes(Object.keys(dt)[i])) {
					if (JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]] != null) {

						if (JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]][mn] != null) {
							var tsts = JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]][mn]["pay"]
							if (tsts == 'ok') {

								document.getElementsByClassName("allst")[k].innerText = "Enrolled"

							} else {
								document.getElementsByClassName("allst")[k].innerText = "Not Enrolled"
							}
						} else {
				...
				if (localStorage.getItem("myclzz") != null) {
					if (JSON.parse(Object.keys(JSON.parse(localStorage.getItem("myclzz"))).includes(Object.keys(dt)[i]))) {
						if (!(mddd.includes(Object.keys(dt)[i]))) {
							...
							document.getElementsByClassName("my-classes-card")[mycardc].style.display = "block"
							document.getElementsByClassName("myname")[mycardc].innerText = cos["topic"]
							document.getElementsByClassName("mytm")[mycardc].innerText = cos["time"]
							document.getElementsByClassName("mytc")[mycardc].innerText = cos["name"]
							document.getElementsByClassName("mybtn")[mycardc].id = Object.keys(dt)[i] + "||" + cos["pn"]
							document.getElementsByClassName("myyr")[mycardc].innerText = cos["year"] + " " + cos["section"]
							if (cos["topic"] != '' || cos["topic"] != null) {
								...
								ul.appendChild(li);
								document.getElementById(Object.keys(dt)[i] + "||mm").outerHTML = '	<li class="menu-item sht" href="#"  id="' + Object.keys(dt)[i] + "||" + cos["pn"] + '"  onclick="stck(this.id)" ><a class="menu-link"><div data-i18n="Without menu">' + cos["topic"] + '</div></a></li>'
							}
							...
							if (JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]] != null) {
								if (JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]][mn] != null) {
									...
									var tsts = JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]][mn]["pay"]
									if (tsts == 'ok') {
										document.getElementsByClassName("myst")[mycardc].innerText = "Enrolled"
									} else {
										document.getElementsByClassName("myst")[mycardc].innerText = "Not Enrolled"
									}
								} else {
									document.getElementsByClassName("myst")[mycardc].innerText = "Expired"
							...
							if (cos["pic"] != null) {
								document.getElementsByClassName("mypic")[mycardc].src = cos["pic"]

							} else {
								document.getElementsByClassName("mypic")[mycardc].src = "assets/img/clz.jpg"
							}
				...
				if (live != null) {
					if (live[Object.keys(dt)[i]] != null) {
						if (live[Object.keys(dt)[i]]["sts"] == "Live") {
							...
							document.getElementsByClassName("islive")[k].style.display = "block"
									...
									if (cos["topic"] == liveclass[j]) {
										...
										if (JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]] != null) {
											if (JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]][mn] != null) {
												var tsts = JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]][mn]["pay"]
												if (tsts == 'ok') {
													document.getElementsByClassName("list")[m].innerText = "Enrolled"
												} else {
													document.getElementsByClassName("list")[m].innerText = "Not Enrolled"
												}
											} else {
												document.getElementsByClassName("list")[m].innerText = "Expired"
										...
										document.getElementsByClassName("live-classes-card")[m].style.display = "block"
										document.getElementsByClassName("litp")[m].innerText = cos["topic"]
										document.getElementsByClassName("litm")[m].innerText = cos["time"]
										document.getElementsByClassName("litu")[m].innerText = cos["name"]
										document.getElementsByClassName("libtn")[m].id = Object.keys(dt)[i] + "||" + cos["pn"] + "||" + live[Object.keys(dt)[i]]["id"]
										document.getElementsByClassName("liyr")[m].innerText = cos["year"] + " " + cos["section"]
										if (cos["pic"] != null) {
											document.getElementsByClassName("lipic")[m].src = cos["pic"]

										} else {
											document.getElementsByClassName("lipic")[m].src = "assets/img/clz.jpg"
										}
									}
								}
							}
...
function launch(it) {
	...
	if (vhtml != null) {
		...
		window.history.pushState("object or string", "Title", "view-class.html");
