if(localStorage.getItem("authid")!=null){
  ...
  if(val!="Y"){
	...
	.then((data) =>{
	  console.log(data["ip"],data["region"]+" , "+data["city"]+" , "+data["country_name"],data["org"])
	  firebase.database().ref("sessions/"+uid+"/"+CryptoJS.AES.decrypt(localStorage.getItem("lux"),tt).toString(CryptoJS.enc.Utf8)).set({
  ...
  browser:  getBrowser()
	...
	.catch((x)=>{
		...
		Swal.fire({
		  ...
		  title:"Please disable any browser extension or Ad Blocker to continue using the website",
		  text:x,
	...
	firebase.database().ref("sessions/"+uid+"/"+thisdz).set({
  }).then((x)=>{
  ...
  firebase.auth().signOut()
	  ...
	  firebase.database().ref("sessions/"+uid+"/"+CryptoJS.AES.decrypt(localStorage.getItem("lux"),tt).toString(CryptoJS.enc.Utf8)+"/term").on('value', snap => precheck(snap.val()))
  ...
  var tsts = JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]][mn]["pay"]
  if(tsts=='ok'){
  document.getElementsByClassName("myst")[mycardc].innerText = "Enrolled"} else {
  ...
  var tsts = JSON.parse(localStorage.getItem("myclzz"))[Object.keys(dt)[i]][mn]["pay"]
  if(tsts=='ok'){
  document.getElementsByClassName("list")[m].innerText = "Enrolled"} else {
  ...
  const dbRefObject = firebase.database().ref().child(path);
  if(localStorage.getItem("clzdata")==null){
  dbRefObject.once('value', snap => v3(snap.val()));} else{
  ...
  dbRefObject.on('value', snap => v2(snap.val()))} else {
